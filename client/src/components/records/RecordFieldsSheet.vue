...
import { ArrowDown, ArrowUp, GripVertical, PencilLine, Plus, Trash2 } from "@lucide/vue"
import { ref } from "vue"
...
const props = defineProps<{ fields: Field[], recordLabel: string }>()
  ...
  <Sheet v-model:open="open">
    <SheetContent class="records-fields-sheet">
      <header class="records-fields-header">
        ...
        <SheetDescription>The order here is the column order everyone starts from. Filter, Show and Search set what readers get in the DAM.</SheetDescription>
      </header>
      <div class="records-fields-body">
        <p v-if="!fields.length" class="admin-text-secondary">No field yet. Add one here, or import a CSV file: each of its columns becomes a text field.</p>
