...
<template>
 <div class="page foundations-page">
  ...
  <div v-if="selected === 'Foundations'" class="reference-content">
   ...
   <section class="geometry-section"><div><h2>Spacing and corner radius</h2><p>Spacing uses a 4 px rhythm. Corner radius depends on the element’s role.</p><dl class="geometry-rules"><div><dt>0 px radius · controls and data</dt><dd>Buttons, inputs, navigation items, interactive cards, badges, charts, progress bars and tables.</dd></div><div><dt>8 px radius · graphics</dt><dd>Colour samples, asset artwork, decorative icon backgrounds and brand imagery.</dd></div><div><dt>12 px radius · layout surfaces</dt><dd>Passive content panels, modal shells and toasts that group information or frame a task.</dd></div></dl></div><div><div class="spacing-label">4 px spacing scale</div><div class="spacing-examples" aria-label="4 pixel spacing scale"><div v-for="value in [4,8,12,16,24,32,48]" :key="value"><span :style="{ height: `${value}px` }"></span><small>{{ value }}</small></div></div></div></section>
  </div>
