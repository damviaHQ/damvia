...
| `asset/assign-products-to-asset-files` | `*/5 * * * *` (every 5 minutes) | The old matcher: applies `PRODUCT_MATCHING_REGEX` to the file names that no matching step owns (files of a type without steps or not related to records, and not linked by hand) and links them to the record with the captured key. Does nothing when `ENABLE_LEGACY_PRODUCT_MATCHING=false`. The matching steps run in the enrichment pass after each sync instead, see [Records](../administration/records.md). |
...
| `asset/extract-metadata` | The `metadata:backfill` CLI command | Reads the EXIF and IPTC metadata of one processed image from its copy in the assets bucket and replaces its stored values. Does nothing for a file that is not `up_to_date` or not an image. Images downloaded by `asset/update-content` are read there, without this job. |
