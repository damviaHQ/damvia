...
The [OneDrive safety guarantees](onedrive.md#safety-and-compatibility-guarantees) apply to Dropbox in the same terms, with `content_hash` in place of `eTag`, "dot-prefixed component in the path" in place of "dot-prefixed name", "folder with no non-empty file below it" in place of "size 0", and the `Dropbox` folder standing in for the drive root when no path is pointed at. Existing libraries depend on these rules; treat a change as a storage migration and test it on a restored copy.
