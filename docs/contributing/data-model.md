...
| `PageBlock` (`page-block.ts`) | `page_blocks` | `pageId`, `type`, `position` (integer), `size` (`full`, `half` or `third`), `data` (`jsonb`, not null) | `page` cascades on delete; index on (`pageId`, `position`). Enum `PageBlockType`: `hero`, `collections`, `files`, `last_files`, `text`, `image`, `video`, `products`. Each type's `data` is defined by a zod schema in `server/src/page-blocks/schema.ts`, which the client imports so both sides validate the same shape |
...
| `DataRecord` (`data-record.ts`) | `records` | `recordKey` (unique across tables), `keyColumnName`, `tableId` (not null, `RESTRICT` on delete; a `BEFORE INSERT` trigger `records_default_table` sets the first table, creating one named `Records` if there is none, when an insert leaves it out), `metaData` (`hstore`, mapped to an object) | `assetFiles`. Indexed on `(table_id, created_at)`. Named `DataRecord` because `Record` would shadow the TypeScript utility type |
| `RecordTable` (`record-table.ts`) | `record_tables` | `name` (unique), `position` | Groups records like the sheets of a workbook; only the admin grid and import use it. Search, facets, collections and links ignore tables |
| `RecordTableAttribute` (`record-table-attribute.ts`) | `record_table_attributes` | Primary key (`tableId`, `attributeId`), `position` | The fields a table shows, in order; both foreign keys cascade on delete. Fields themselves stay shared by the catalogue |
...
`enrichment_settings.related_records` stores the nullable global display rules for related records; `collections.related_records` stores a nullable override. Null on a collection inherits the global default. Null globally uses same-model records across accessible lists.

The JSON object contains `enabled` and up to five `groups`. Each group has `scope` (`current` or `all`), `matchField` (`$family`, a visible field name, or null), `hasPhoto`, `filters` and `excludeFilters`. Filters within a group combine with AND; any matching exclusion rejects that group's candidate; groups combine with OR. Each filter array is capped at ten. These settings do not write `collection_records`. Candidate queries preserve visibility, omit the source record before pagination and count only permitted image files for `hasPhoto`.

## Pictures accessible through records

`collection_files.collection_id` can be null. One such row per image asset provides a stable file identity for previews, favourites and downloads when no file collection holds it. A partial unique index enforces that identity; asset insert/MIME-update triggers and the migration backfill create it. The row is not a grant and does not change collection file counts. `userCollectionFilesQuery` checks live record links, accessible record membership (including whole-catalogue lists), exclusions, invitations and licences on every read. Moving a source file preserves this identity. Deleting the asset removes it. Ordinary rows still represent explicit membership of file collections.
