...
| `AssetType` (`asset-type.ts`) | `asset_types` | `name`, `description`, `isRelatedToRecords`, `includeInSearchByDefault`, `defaultDisplay` (`grid` or `list`), `listDisplayItems` (`text[]`) | No relations declared |
...
| `EnrichmentSettings` (`enrichment-settings.ts`) | `enrichment_settings` | Single row, `id` = 1: `recordLabelSingular`, `recordLabelPlural` | No relations; read by the public `env` query |
| `RecordAttribute` (`record-attribute.ts`) | `record_attributes` | `name` (unique), `displayName`, `facetable`, `searchable` (indexed), `viewable` | No relations declared |
