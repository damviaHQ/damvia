...
4. Recomputes `sample_file_ids` for every collection: the first four collection files with a thumbnail, ordered by depth in the collection tree then creation date. These are the four images in a collection's thumbnail mosaic.
