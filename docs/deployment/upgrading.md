...
| `1791244800000-record-tables` | `record_tables` and `record_table_attributes`, and `table_id` (not null) on `records`. One table named after the record label, plural, receives every record and shows every field. A trigger puts a record inserted without a table in the first table. Rolling back drops the tables and the column: records stay, their grouping is lost |
