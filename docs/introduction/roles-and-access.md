...
description: The four user roles, the account gates, and the exact rule that decides which collections and files a user can see or edit.
sidebar:
  order: 3
...
| `guest` | Created automatically by an invitation | Only invited collections, collections limited to a group they belong to, and their own | Downloads only; no favorites, no "add to collection", no menu |
...
Guests skip both gates: an invitation creates them with `approved` and `emailVerified` already `true`. See [Users and approval](../administration/users-and-approval.md).
...
`userCollectionFilesQuery` applies the same five branches to individual files, except that branch 3 checks the file's own license rather than the folder's. A licensed file inside an unlicensed collection is therefore still hidden from a region the license excludes.

:::caution
Branch 4 applies to guests too. Because a guest receives the default group of the inviter's region, any collection limited to that default group becomes visible to every guest invited from that region. Keep default groups out of `limitedToGroupIds` unless that is intended.
:::
...
Any approved user who can edit a collection can invite an email address to it with an expiry date and optionally send the `mailer/invitation` email. If the address matches an existing user, that user gains access through branch 5 with their current role. If not, a guest account is created with `name` and `company` set to `NA`, the inviter's region, that region's default group, and both account gates already passed.

A guest logs in like any other user (password or magic link when `ENABLE_PASSWORD_LESS_AUTH=true`). In the client, a guest sees the collections they can access (other than their own) instead of the menu, lands on the first of them, and can select and download files. The favorites endpoints reject guests server-side (`userMember`), and the "add to collection" action is hidden. Removing the invitation, or letting it expire, removes the access; deleting a user also deletes every invitation addressed to their email.
