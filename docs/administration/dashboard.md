...
3. When the measurement shows less usage than the previous one and the sync was paused, the worker queues `asset/update-content` for every `creating` and `outdated` file on its own and logs `storage.quota-recovered`. "Retry pending files" does the same by hand and says how many files will be downloaded again. It is only available when no file download is waiting or running: the button then reads "Downloading N files…", and the server refuses the action with `Files are already being downloaded. Try again once they are done.` The cloud sync queues new files every 5 minutes, so the button is often disabled for a moment right after a sync pass.
