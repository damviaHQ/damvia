...
test('the router validates the pattern as a field error, refuses duplicates, lists counts, examples and overlaps, previews what a save would change and applies on save', async () => {
    ...
    assert.deepEqual([preview.matches, preview.applied, preview.keptByHand, preview.lostTo], [2, 2, 0, []])
    ...
    assert.deepEqual([manual.matches, manual.applied, manual.keptByHand], [1, 0, 1])
    assert.deepEqual(manual.changes, { folders: 0, files: 0, groups: [] })
    const tied = await admin.assetTypeRule.preview({ pattern: '^/Router root$', assetTypeId: other.id })
    assert.deepEqual([tied.matches, tied.applied, tied.lostTo], [1, 0, [{ rulePattern: '^/Router root', count: 1 }]])
    ...
    assert.ok(listed.roots.includes('/Router root'))
    assert.ok(!listed.roots.includes('/Router root/Shots'))
    const mine = listed.rules.find(r => r.id === created.rule.id)
    assert.deepEqual([mine.folders, mine.keptByHand, mine.examples, mine.overlaps, mine.lastError], [1, 1, ['/Router root'], [twin.rule.id], null])
